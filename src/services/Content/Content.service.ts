...
import { TCreateContentData } from '../../types/content'
import { BadRequest400Error, Forbidden403Error, NotFound404Error } from '../../utils/error.class'
...
export default class ContentService implements TContentService {
  ...
  getOne: TContentService['getOne'] = async (contentId) => {
    const content = await this.repo.getOne(new Validator.num(contentId, 'contentId').value())
  ...
  private getVideoData = async (videoUrl: string) => {
    ...
    return {
      ...
      thumbnailUrl: new Validator.str(thumbnail_url, 'thumbnailUrl').notEmpty().value(),
      creatorName: new Validator.str(author_name, 'creatorName').notEmpty().value(),
      creatorUrl: new Validator.str(author_url, 'creatorUrl').notEmpty().value(),
    }
  }
